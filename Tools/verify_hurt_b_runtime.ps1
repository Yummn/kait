...
foreach ($kind in @('hurt', 'hit', 'hit-overlap', 'kill', 'chain', 'block', 'push', 'gameplay')) {
    $shot = Join-Path $project "VFXScreenshots\hurt-b-$kind.png"
    $log = Join-Path $project "Logs\hurt-b-$kind-player.log"
    $started = Get-Date
    ...
    if ($process.ExitCode -ne 0) { throw "Player failed: $kind, exit=$($process.ExitCode)" }
