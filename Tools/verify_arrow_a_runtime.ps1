...
foreach ($kind in @('arrow', 'arrow-left', 'arrow-up', 'arrow-down', 'arrow-kait', 'hit', 'gameplay')) {
    $shot = Join-Path $project "VFXScreenshots\arrow-a-$kind.png"
    $log = Join-Path $project "Logs\arrow-a-$kind-player.log"
    $started = Get-Date
