...
foreach ($kind in @('phantom', 'phantom-move', 'phantom-release', 'phantom-death', 'phantom-reset', 'ice', 'dread', 'hit', 'gameplay')) {
    $shot = Join-Path $project "VFXScreenshots\phantom-b-$kind.png"
    $log = Join-Path $project "Logs\phantom-b-$kind-player.log"
    $started = Get-Date
    ...
    $errors = Select-String -LiteralPath $log -Pattern 'Phantom QA:|Assertion failed|AssertionException|NullReferenceException|Shader error|IndexOutOfRangeException|MissingReferenceException'
