...
foreach ($kind in @('dread', 'dread-left', 'dread-up', 'dread-down', 'boundary', 'boundary-left', 'boundary-up', 'boundary-down', 'hit', 'gameplay')) {
    $shot = Join-Path $project "VFXScreenshots\dread-a-$kind.png"
    $log = Join-Path $project "Logs\dread-a-$kind-player.log"
    $started = Get-Date
