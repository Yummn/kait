...
foreach ($kind in @('ice', 'ice-move', 'ice-release', 'ice-death', 'dread', 'boundary-up', 'boundary-down', 'hit', 'gameplay')) {
    $shot = Join-Path $project "VFXScreenshots\ice-a-$kind.png"
    $log = Join-Path $project "Logs\ice-a-$kind-player.log"
    $started = Get-Date
