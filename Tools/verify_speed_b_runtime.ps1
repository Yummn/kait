...
foreach ($kind in @('speed-low', 'speed-mid', 'speed-high', 'speed-high-left', 'speed-high-move', 'boundary', 'hit', 'gameplay')) {
    $shot = Join-Path $project "VFXScreenshots\speed-b-$kind.png"
    $log = Join-Path $project "Logs\speed-b-$kind-player.log"
    $started = Get-Date
