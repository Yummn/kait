...
$snapshot | Export-Clixml -LiteralPath (Join-Path $project 'Logs/character-cg-playerprefs-backup.xml')
try {
    $proc = Start-Process -FilePath (Join-Path $project 'Build/kait.exe') -ArgumentList '-screen-fullscreen 0 -screen-width 1920 -screen-height 1080 -kaitCharacterCGQA 1 -kaitScreenshot C:/Users/yummn/Downloads/kait/Logs/character-cg -logFile C:/Users/yummn/Downloads/kait/Logs/character-cg-runtime.log' -WindowStyle Hidden -PassThru
...
$log = Get-Content -LiteralPath (Join-Path $project 'Logs/character-cg-runtime.log')
if (!($log -match 'CHARACTER_CG_QA_COMPLETE') -or ($log -match 'CHARACTER_CG_QA:|NullReferenceException|MissingReferenceException|Shader error')) { throw 'Runtime QA failed; inspect log.' }
Write-Output 'CHARACTER_CG_QA_COMPLETE'
