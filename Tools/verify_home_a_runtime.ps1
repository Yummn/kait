...
try {
    $proc = Start-Process -FilePath (Join-Path $project 'Build/kait.exe') -ArgumentList '-screen-fullscreen 0 -screen-width 1920 -screen-height 1080 -kaitMenuPreview home -kaitScreenshot C:/Users/yummn/Downloads/kait/Logs/home-a -logFile C:/Users/yummn/Downloads/kait/Logs/home-a-runtime.log' -WindowStyle Hidden -PassThru
...
$log = Get-Content -LiteralPath (Join-Path $project 'Logs/home-a-runtime.log')
if (!($log -match 'HOME_QA_COMPLETE') -or ($log -match 'HOME_QA:|Main menu QA:|NullReferenceException|MissingReferenceException|Shader error')) { throw 'Runtime QA failed; inspect log.' }
Write-Output 'HOME_QA_COMPLETE'
