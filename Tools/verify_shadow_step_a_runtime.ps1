...
foreach ($kind in @('shadow', 'phantom', 'ice', 'hit', 'gameplay')) {
    $shot = Join-Path $project "VFXScreenshots\shadow-step-a-$kind.png"
    $log = Join-Path $project "Logs\shadow-step-a-$kind-player.log"
    $started = Get-Date
    ...
    $errors = Select-String -LiteralPath $log -Pattern 'Shadow Step QA:|Phantom QA:|Assertion failed|AssertionException|NullReferenceException|Shader error|IndexOutOfRangeException|MissingReferenceException'
