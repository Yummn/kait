...
foreach ($kind in @('landing', 'landing-sword', 'landing-archer', 'landing-guard', 'landing-mage', 'landing-boss', 'landing-cancel', 'gameplay')) {
    $shot = Join-Path $project "VFXScreenshots\landing-a-$kind.png"
    $log = Join-Path $project "Logs\landing-a-$kind-player.log"
    $started = Get-Date
