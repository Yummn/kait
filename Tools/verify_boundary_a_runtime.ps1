...
foreach ($kind in @('boundary', 'boundary-left', 'boundary-up', 'boundary-down', 'landing', 'hit', 'gameplay')) {
    $shot = Join-Path $project "VFXScreenshots\boundary-a-$kind.png"
    $log = Join-Path $project "Logs\boundary-a-$kind-player.log"
    $started = Get-Date
