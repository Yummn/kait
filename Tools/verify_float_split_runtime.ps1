...
$snapshot | Export-Clixml -LiteralPath (Join-Path $project 'Logs/float-split-playerprefs-backup.xml')
try {
    $proc = Start-Process -FilePath (Join-Path $project 'Build/kait.exe') -ArgumentList '-screen-fullscreen 0 -screen-width 1920 -screen-height 1080 -kaitYummn082QA 1 -kaitScreenshot C:/Users/yummn/Downloads/kait/Logs/float-split -logFile C:/Users/yummn/Downloads/kait/Logs/float-split-runtime.log' -WindowStyle Hidden -PassThru
...
$log = Get-Content -LiteralPath (Join-Path $project 'Logs/float-split-runtime.log')
