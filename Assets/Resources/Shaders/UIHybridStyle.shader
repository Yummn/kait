    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag(v2f input) : SV_Target
            {
                ...
                float2 roundedPoint = abs((input.style.xy - 0.5) * size) - (size * 0.5 - radius);
                float roundedDistance = length(max(roundedPoint, 0)) + min(max(roundedPoint.x, roundedPoint.y), 0) - radius;
                float antialias = max(fwidth(roundedDistance), 0.65);
                color.a *= 1 - smoothstep(-antialias, antialias, roundedDistance);
