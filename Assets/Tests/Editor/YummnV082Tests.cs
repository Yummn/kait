...
public class YummnV082Tests
{
    ...
    {var one=YummnRulesSnapshot.Current(attackOne:true);var copy=JsonUtility.FromJson<YummnRulesSnapshot>(JsonUtility.ToJson(one));Assert.AreEqual(10,copy.AttackCostTenths);Assert.IsFalse(copy.AttackCostsTenth);Assert.AreNotEqual(one.ScoreKey,YummnRulesSnapshot.Current(attackTenth:true).ScoreKey);Assert.AreNotEqual(one.ScoreKey,YummnRulesSnapshot.Current(attackOne:false).ScoreKey);StringAssert.Contains("攻击耗气",YummnTutorial.ForRules(copy)[0].Title);}
    [Test] public void NewPreferenceOverridesLegacyAndDoesNotMutateCurrentRun()
    {
        ...
        try{var current=R().Yummn.rules;var method=typeof(KaitGame).GetMethod("YummnPreset",BindingFlags.Static|BindingFlags.NonPublic);PlayerPrefs.DeleteKey(key);PlayerPrefs.SetInt(old,1);Assert.AreEqual(10,((YummnRulesSnapshot)method.Invoke(null,null)).AttackCostTenths);PlayerPrefs.SetInt(key,0);Assert.AreEqual(0,((YummnRulesSnapshot)method.Invoke(null,null)).AttackCostTenths);PlayerPrefs.SetInt(key,1);Assert.AreEqual(10,((YummnRulesSnapshot)method.Invoke(null,null)).AttackCostTenths);Assert.AreEqual(10,current.AttackCostTenths);}
    ...
    [Test] public void T08_CrossConsumesTwoMarkersAndEachRestoresOne()
    {var r=R();r.Yummn.ki=0;Marker(r,2,2);Marker(r,3,2);var e=E(r,4,4,2,KaitEnemyType.Warlock);Aim(e,KaitIntentType.CrossBlast,new Vector2Int(2,2),new Vector2Int(3,2));Call(r,"ResolveYummnEnemyPhase",Context(r));Assert.AreEqual(2,r.Ki);Assert.AreEqual(2,r.Yummn.metrics.afterimageKi);Assert.AreEqual(2,r.Yummn.metrics.afterimagesHit);}
    ...
    [Test] public void T27_LockedBoardStaysIdleNoReset(){var r=R();Locked(r);var before=(int[,])r.threat.Clone();r.TryGlobalInput(KaitDirection.Right);Assert.IsFalse(r.ended);Assert.AreEqual(string.Empty,r.endReason);CollectionAssert.AreEqual(before,r.threat);}
    ...
    [Test] public void TutorialCurrentHasFourPagesAndCorrectRules(){var pages=YummnTutorial.ForRules(YummnRulesSnapshot.Current());Assert.AreEqual(4,pages.Length);StringAssert.Contains("上限7",pages[0].Lead);StringAssert.Contains("击杀+3",pages[0].Lead);StringAssert.Contains("残影",pages[1].Title);}
}
