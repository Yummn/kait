...
public sealed partial class KaitGame
{
    ...
    private static YummnRulesSnapshot YummnPreset()=>YummnRulesSnapshot.Current(
        Mathf.Clamp(PlayerPrefs.GetInt(Yummn082Preference+"MaxKi",6),3,6),
    ...
    private void AddYummnSettings(Transform parent)
    {
        ...
        for(int i=0;i<4;i++)
        {
            int index=i;var b=MakeFlatButton(parent,new Vector2(0,121-i*40),new Vector2(580,36),"");
            ...
            {
                var r=YummnPreset();int value=index==0?(r.MaxKi==3?6:r.MaxKi-1):index==1?(r.MovementCostMode==YummnMovementCostMode.PerCell?1:0):index==2?(r.KillKi==1?3:r.KillKi-1):((int)r.Supply+1)%4;
                PlayerPrefs.SetInt(Yummn082Preference+keys[index],value);PlayerPrefs.Save();RefreshYummnSettingsNote();
            });
        }
        var next=YummnPreset();
        yummnAttackPhaseToggle=AddYummnRuleToggle(parent,-30,"攻击也推进敌方回合","AttackPhase",next.AttackAdvancesEnemyPhase);
        yummnFullRecoveryToggle=AddYummnRuleToggle(parent,-70,"气竭需要回满气","FullRecovery",next.ExhaustionNeedsFullKi);
        yummnAttackCostToggle=AddYummnRuleToggle(parent,-110,"高速攻击耗气：0气 / 1气","AttackOne",next.AttackCostsOne);
        AddYummnRuleToggle(parent,-150,"有效移动额外推进敌方回合","MovePhase",next.MovementAdvancesEnemyPhase);
        yummnEightToggle=AddYummnRuleToggle(parent,-190,"从8开始出怪（默认4）","FromEight",next.SpawnFromEight);
        yummnSettingsNote=MakeText("",parent,new Vector2(0,-265),new Vector2(680,70),17,Peach,TextAnchor.MiddleCenter,FontStyle.Normal,false);yummnSettingsNote.font=threatBoardFont;
