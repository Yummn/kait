...
public static class YummnComicTutorial
{
    public static KaitTutorialPages.Page[] ForRules(YummnRulesSnapshot r)
    {
        ...
        string attack=r.AttackCostTenths==0?"高速贴身拳不耗气；气竭改用踢击。":$"高速每拳耗{r.AttackCostTenths/10f:0.#}气；气竭踢击免费。";
    ...
    public static string Appendix(YummnRulesSnapshot r)
    {
        ...
        s.AppendLine("气竭普通攻击改为免费、1伤的踢击，不触发疾风连击、推掌、震慑拳等拳掌效果。一次操作沿用起手时的高速/气竭资格；中途耗尽或回气都在全部结算结束后切换状态。");
        ...
        s.AppendLine("伺机而动：高速行动中，敌人进入四邻后拳击，可联动疾风连击；气竭起手不触发。借机攻击：敌人离开四邻后踢击，不触发拳击技能，气竭免费。人物移动、传送、敌人移动、推动都按相邻关系变化判断；两者分别每敌人每次操作限一次，死亡不算离开。");
        ...
        s.AppendLine("远古奥秘之书：任意数字满5枚合成最旧两枚。共鸣水晶只合成本次新产生且仍在盘上的同值结果。重力摆锤每次操作最多反向滑一次。");
        s.AppendLine("守卫刻文和魔法飞弹不是拳击，不触发疾风连击；法术回响只复制本次操作前面的原始合成攻击。丰饶之壶等所有连锁结束后返还原料，无空位直接丢弃。法师之手点副盘数字删除，只耗1气，不移动、不补2、不触发合成。");
        s.AppendLine("镜影术点空格生成残影；粉碎音波点目标格攻击十字范围；雷鸣波点自身推开四邻；命令术·出手点敌人，只让该敌人行动一次。主动技能的标注是总气费。");
        s.AppendLine("残影默认承受一次攻击，在敌方回合末消退。持久幻影延长到两个敌方回合；暗影斗篷使其在存续回合内耐受多次攻击。镜影共鸣每次攻击每个残影只结算一次；暗影反击按每个受击残影或诱饵反伤。误导术不改变敌人已经锁定的攻击。");
